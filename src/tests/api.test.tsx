...
describe('sapi integration', () => {
  ...
  it('fetches and displays cats after search', async () => {
    ...
    expect(api.fetchCats).toHaveBeenCalledWith('british');
  ...
  it('fetches and displays message if no cats found', async () => {
    ...
    expect(api.fetchCats).toHaveBeenCalledWith('unknown cat');
    expect(await screen.findByText(messages.noCatsFound)).toBeInTheDocument();
  });

  it('handles fetch error', async () => {
    (api.fetchCats as ReturnType<typeof vi.fn>).mockRejectedValue({
    ...
    expect(api.fetchCats).toHaveBeenCalled();
